...
class EcommerceBrandPage extends StatefulWidget {
  const EcommerceBrandPage({super.key});

  @override
  _EcommerceBrandPageState createState() => _EcommerceBrandPageState();
}

class _EcommerceBrandPageState extends State<EcommerceBrandPage>
    with SingleTickerProviderStateMixin, UIMixin {
  late EcommerceBrandController controller;

  @override
  void initState() {
    super.initState();
    controller = Get.put(EcommerceBrandController());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Layout(
        child: GetBuilder<EcommerceBrandController>(
            init: controller,
            builder: (controller) {
              return Column(
                      ...
                      children: [
                        ...
                        MyBreadcrumb(
                          children: [
                            ...
                            MyBreadcrumbItem(name: 'Brand'.tr(), active: true),
                          ],
                        ),
                      ],
                    ),
                      ...
                      child: Column(
                        ...
                        children: [
                          MyButton(
                            onPressed: () {
                              Navigator.pushNamed(
                                  context, '/apps/ecommerce/add_brand');
                            },
                            elevation: 0,
                                  ...
                                  children: [
                                    MyContainer.none(
                                      ...
                                      child: DataTable(
                                        ...
                                        rows: controller.brandData.map((brand) {
                                          return DataRow(cells: [
                                            DataCell(Text('${brand.id}')),
                                            DataCell(Text(
                                                brand.languages!.first.name!)),
                                            DataCell(
                                              NetworkImageWidget(
                                                  width: 25,
                                                  height: 25,
                                                  imageUrl:
                                                  'https://swan.alisonsnewdemo.online/images/manufacturer/${brand.image}'),
  ...
  Widget _buildPaginationControls(EcommerceBrandController categoryProvider) {
    return Padding(
